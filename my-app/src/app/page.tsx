...
const Home: React.FC = () => {
 ...
 const [bases] = useState<BasesType>({
  ...
  const [campanhasComAviso, setCampanhasComAviso] = useState<string[]>([]); // Atualizado para ser uma matriz de strings
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const newCampanhasComAviso: string[] = []; // Crie um novo array para campanhas com aviso
        ...
        for (const baseName in bases) {
          ...
          const temPorcentagemMaiorQue90 = filteredData.some((post: PostType) => {
            const percentage = parseFloat(post.completed_percentage);
            return !isNaN(percentage) && percentage > 90;
          });
  
          if (temPorcentagemMaiorQue90) {
            newCampanhasComAviso.push(id);
          }
        }
  
        setCampanhasComAviso(newCampanhasComAviso); // Atualize campanhasComAviso com o novo array
        setBaseData(newData);
        setLoading(false);
